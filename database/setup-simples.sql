-- V8 System - Setup Simplificado (SEM TRIGGERS COMPLEXOS)
-- Execute este arquivo no SQL Editor do Supabase

-- 1. LIMPAR TABELAS EXISTENTES (OPCIONAL)
-- ========================================
-- Descomente as linhas abaixo se quiser limpar tudo antes
...
-- Veículos (SEM TRIGGERS COMPLEXOS)
CREATE TABLE IF NOT EXISTS veiculos (
    ...
    classe_social CHAR(1) DEFAULT 'D' CHECK (classe_social IN ('A','B','C','D')),
    km INTEGER DEFAULT 0,
    cor VARCHAR(30),
    combustivel VARCHAR(20) DEFAULT 'flex' CHECK (combustivel IN ('flex','gasolina','diesel','eletrico','hibrido')),
    cambio VARCHAR(20) DEFAULT 'manual' CHECK (cambio IN ('manual','automatico','cvt')),
...
CREATE TABLE IF NOT EXISTS veiculo_fotos (
    ...
    tipo VARCHAR(20) DEFAULT 'galeria' CHECK (tipo IN ('principal','galeria','vitrine')),
    ordem INTEGER DEFAULT 0,
...
-- 5. INSERIR VEÍCULOS DE EXEMPLO
-- ===============================

-- Obter IDs das categorias e inserir veículos
DO $$
DECLARE
    cat_hatch INTEGER;
    cat_sedan INTEGER;
...
BEGIN
    ...
    SELECT id INTO cat_hatch FROM categorias WHERE slug = 'hatch' LIMIT 1;
    SELECT id INTO cat_sedan FROM categorias WHERE slug = 'sedan' LIMIT 1;
    SELECT id INTO cat_suv FROM categorias WHERE slug = 'suv' LIMIT 1;
    SELECT id INTO cat_picape FROM categorias WHERE slug = 'picape' LIMIT 1;
    SELECT id INTO cat_esportivo FROM categorias WHERE slug = 'esportivo' LIMIT 1;

    -- Verificar se já existem veículos
    IF NOT EXISTS (SELECT 1 FROM veiculos LIMIT 1) THEN
        
        -- CLASSE A (Acima de R$ 80k)
        INSERT INTO veiculos (marca, modelo, ano, valor, categoria_id, classe_social, km, cor, combustivel, cambio, portas, placa, codigo_interno, observacoes) 
        VALUES
        ...
        ('Porsche', '911 Carrera', 2021, 850000, COALESCE(cat_esportivo, cat_sedan), 'A', 12000, 'Amarelo', 'gasolina', 'automatico', 2, 'JKL3456', 'POR001', 'Esportivo icônico');
        ...
        ('Nissan', 'Frontier LE', 2023, 72000, COALESCE(cat_picape, cat_suv), 'B', 22000, 'Vermelho', 'diesel', 'automatico', 4, 'VWX9012', 'NIS001', 'Picape robusta, 4x4');
        ...
        UPDATE veiculos SET dias_estoque = DATE_PART('day', CURRENT_DATE - data_entrada)::INTEGER;
        ...
        -- Simular algumas reservas
        UPDATE veiculos 
        SET status = 'reservado' 
        WHERE codigo_interno IN ('BMW001', 'TOY001');
        
        RAISE NOTICE 'Veículos de exemplo inseridos com sucesso!';
    ELSE
        RAISE NOTICE 'Veículos já existem no banco.';
    END IF;
END $$;
...
-- Criar políticas públicas para leitura
...
-- Para desenvolvimento, permitir todas operações (REMOVER EM PRODUÇÃO!)
CREATE POLICY "dev_all_loja" ON loja_info FOR ALL USING (true);
CREATE POLICY "dev_all_classes" ON classes_config FOR ALL USING (true);
CREATE POLICY "dev_all_categorias" ON categorias FOR ALL USING (true);
CREATE POLICY "dev_all_veiculos" ON veiculos FOR ALL USING (true);
CREATE POLICY "dev_all_fotos" ON veiculo_fotos FOR ALL USING (true);
...
BEGIN
    ...
    RAISE NOTICE '✅ Setup do V8 System concluído!';
    RAISE NOTICE '📊 Categorias criadas: %', total_categorias;
    RAISE NOTICE '🚗 Veículos inseridos: %', total_veiculos;
